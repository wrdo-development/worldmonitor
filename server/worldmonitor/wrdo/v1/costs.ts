/**
 * GET /api/wrdo/v1/costs
 *
 * Proxies to Heart dashboard metrics for token/cost telemetry.
 * Auth: x-node-secret (CAVE_NODE_SECRET) + x-wrdo-product: cave
 *
 * Optional query params forwarded to Heart:
 *   - userId  (forwarded as x-wrdo-user-id header if present)
 */
...
export function createCostsRoute(): RouteDescriptor {
  return {
    method: 'GET',
    path: '/api/wrdo/v1/costs',
    async handler(req: Request): Promise<Response> {
      ...
      if (userId) {
        upstreamHeaders['x-wrdo-user-id'] = userId;
      }

      try {
        const upstream = await fetch(`${heartUrl}/api/v1/dashboard/metrics`, {
