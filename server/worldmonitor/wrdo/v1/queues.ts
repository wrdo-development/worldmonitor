/**
 * GET /api/wrdo/v1/queues
 *
 * Proxies to Heart API for BullMQ queue health.
...
export function createQueuesRoute(): RouteDescriptor {
  return {
    method: 'GET',
    path: '/api/wrdo/v1/queues',
    async handler(_req: Request): Promise<Response> {
      ...
      try {
        const upstream = await fetch(`${heartUrl}/api/v1/admin/queue-health`, {
