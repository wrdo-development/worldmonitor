/**
 * GET /api/wrdo/v1/calendar
 *
 * Proxies to Heart API for the owner's Google Calendar events.
 * Auth: x-node-secret (CAVE_NODE_SECRET) + x-wrdo-product: cave
 *       x-wrdo-user-id: WRDO_OWNER_USER_ID (defaults to Alwyn's UUID)
 *
 * Heart endpoint: GET /api/v1/google/calendar/events
 */
...
export function createCalendarRoute(): RouteDescriptor {
  return {
    method: 'GET',
    path: '/api/wrdo/v1/calendar',
    async handler(_req: Request): Promise<Response> {
      ...
      try {
        const upstream = await fetch(`${heartUrl}/api/v1/google/calendar/events`, {
